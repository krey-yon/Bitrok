...
export function AuthShell({ eyebrow, title, description, children, asideTitle = "One URL. Every session.", asideBody = "Your frontend keeps the same backend endpoint while your local service disconnects, restarts, and moves with you." }: { eyebrow: string; title: string; description: string; children: React.ReactNode; asideTitle?: string; asideBody?: string }) {
  return (
    <div className="relative min-h-full bg-page-gradient">
      ...
      <main id="main-content" className="relative z-10 mx-auto grid w-full max-w-5xl items-stretch gap-8 px-5 pb-12 pt-6 lg:grid-cols-[.88fr_1.12fr] lg:pb-20 lg:pt-12">
        <section className="order-2 hidden overflow-hidden rounded-[var(--radius-xl)] border border-hairline bg-foreground p-8 text-background lg:order-1 lg:flex lg:min-h-[640px] lg:flex-col lg:justify-between">
          ...
          <div className="grid grid-cols-3 gap-px overflow-hidden rounded-lg border border-background/15 bg-background/15 text-center"><div className="bg-foreground p-3"><strong className="block font-mono text-xs text-accent">TLS</strong><span className="text-[10px] text-background/45">encrypted</span></div><div className="bg-foreground p-3"><strong className="block font-mono text-xs text-accent">HTTP</strong><span className="text-[10px] text-background/45">requests</span></div><div className="bg-foreground p-3"><strong className="block font-mono text-xs text-accent">WS</strong><span className="text-[10px] text-background/45">realtime</span></div></div>
        </section>
